...
local function parse_option_flags(value, flags)
  ...
  for _, flag in ipairs(flags) do
    ...
    if count > 0 then
      ...
      if flag ~= "mock" then
        ...
        sanitized[sanitized_count] = flag
      end
  ...
  return pl_stringx.strip(value), result, table.concat(sanitized, " ")
end
...
local function load(path, custom_conf)
  ...
  do
    ...
    local listeners, err = parse_listeners(conf.proxy_listen, { "proxy_protocol", "mock" })
    if err then
